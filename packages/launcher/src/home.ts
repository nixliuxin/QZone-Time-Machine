...
function card(u){
  ...
  const title=u.remark||u.nickname||u.name||String(u.uin||'');
  // Avatar
  let av;
  ...
  meta.appendChild(nm);
  if(u.remark && u.nickname && u.remark!==u.nickname){ const nk=document.createElement('div'); nk.className='nick'; nk.textContent='昵称：'+u.nickname; meta.appendChild(nk); }
...
const sorted=[...USERS].sort((a,b)=>{ const ca=hasCounts(a)?1:0, cb=hasCounts(b)?1:0; if(cb!==ca)return cb-ca; return (a.remark||a.nickname||'').localeCompare(b.remark||b.nickname||'','zh'); });
function apply(){
  const t=q.value.trim().toLowerCase(); const od=onlyData.checked;
  let list=sorted;
  if(od) list=list.filter(hasCounts);
  if(t) list=list.filter(u=>[u.remark,u.nickname,u.name,String(u.uin||'')].some(v=>(v||'').toLowerCase().includes(t)));
